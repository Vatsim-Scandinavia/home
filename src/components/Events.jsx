        ...
        <div className="flex flex-col w-full h-fit" id="live-stats" style={{ display: loading ? 'none' : 'flex' }}>
            <div className="flex h-full flex-col gap-2" >
                {events.slice(0, 2).map((item, index) => (
                    <a href={item.url} target='_blank' key={item.id || index} className='aspect-video h-1/3 md:h-60 flex dark:hover:!text-primary text-secondary dark:text-white hover:bg-snow transition-all p-2 rounded'>

                        <img aria-label={`View event: ${item.name}`} className='h-full aspect-video bg-center bg-cover rounded' src={item.banner}/>
